...
public class UpscaleRenderer {
    private static int lowResFbo = 0;
    private static int lowResColor = 0;
    private static int lowResDepth = 0;
    private static int width = 0;
    private static int height = 0;
    private static int fsrEasuProgram = 0;
    private static int fsrRcasProgram = 0;
    ...
    public static void ensure(int screenW, int screenH, float scale) {
        ...
        if (lowResFbo == 0 || targetW != width || targetH != height) {
            ...
            lowResFbo = GL30.glGenFramebuffers();
            GL30.glBindFramebuffer(GL30.GL_FRAMEBUFFER, lowResFbo);
            lowResColor = GL11.glGenTextures();
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, lowResColor);
            ...
            lowResDepth = GL30.glGenRenderbuffers();
            GL30.glBindRenderbuffer(GL30.GL_RENDERBUFFER, lowResDepth);
        ...
        if (fsrEasuProgram == 0) {
            fsrEasuProgram = com.example.upscale.gl.ShaderUtil.loadProgram("fsr3_easu.vert", "fsr3_easu.frag");
        }
        if (fsrRcasProgram == 0) {
            fsrRcasProgram = com.example.upscale.gl.ShaderUtil.loadProgram("fsr3_rcas.vert", "fsr3_rcas.frag");
        }
    }
    ...
    public static void presentToScreen(int screenW, int screenH) {
        ...
        GL20.glUseProgram(0);
        // Optional RCAS pass could follow
    }
