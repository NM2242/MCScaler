...
public final class MainSettingsScreen extends SettingsScreen {
    ...
    protected void init() {
        ...
        // Add proper scaling slider like Build Test mod
        scaleSlider = new SliderWidget(centerX - 100, centerY + 60, 200, 20,
                Text.literal("Scale: " + String.format("%.2f", mod.getScaleFactor())),
                mod.getScaleFactor() / 2.0) { // Normalize to 0.0-1.0 range (0.0-2.0 scale)

            @Override
            protected void updateMessage() {
                this.setMessage(Text.literal("Scale: " + String.format("%.2f", this.value * 2.0)));
            }

            @Override
            protected void applyValue() {
                // Convert normalized value (0.0-1.0) back to scale factor (0.0-2.0)
                double scaleValue = this.value * 2.0;
                mod.setScaleFactor((float) scaleValue);
                updateButtons();
            }
        };
