...
public final class MainSettingsScreen extends SettingsScreen {
    ...
    protected void init() {
        ...
        scaleSlider = new SliderWidget(centerX - 100, centerY + 40, 200, 20,
                Text.literal("Scale: " + String.format("%.2f", mod.getScaleFactor())),
                mod.getScaleFactor() / 8.0) { // Normalize to 0.0-1.0 range

            @Override
            protected void updateMessage() {
                this.setMessage(Text.literal("Scale: " + String.format("%.2f", this.value * 8.0)));
            }

            @Override
            protected void applyValue() {
                // Convert normalized value (0.0-1.0) back to scale factor (0.0-8.0)
                double scaleValue = this.value * 8.0;
