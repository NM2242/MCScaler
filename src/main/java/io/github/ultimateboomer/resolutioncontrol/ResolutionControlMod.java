...
public class ResolutionControlMod implements ModInitializer {
	...
	public void onInitialize() {
		...
		// Auto-refresh tick event
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			if (client.world != null && !refreshInProgress && ConfigHandler.instance.getConfig().enableAutoRefresh) {
				long currentTime = System.currentTimeMillis();
				
				// Check if 3 seconds have passed since world load
				if (worldLoadTime > 0 && currentTime - worldLoadTime >= 3000) {
					startAutoRefresh();
					worldLoadTime = 0; // Reset to prevent multiple triggers
				}
			}
		...
		ServerWorldEvents.LOAD.register((server, world) -> {
			...
			// Set world load time for auto-refresh
			worldLoadTime = System.currentTimeMillis();
			LOGGER.info("World loaded, auto-refresh will trigger in 3 seconds");
		});
...
//		resizeMinecraftFramebuffers();
	...
	private void startAutoRefresh() {
		...
		originalScaleFactor = getScaleFactor();
		refreshInProgress = true;
		needsRefresh = true;
		
		LOGGER.info("Starting auto-refresh: moving scale from {} to 0.10", originalScaleFactor);
		setScaleFactor(0.10f);
	}
	
	private void handleRefreshSequence() {
		if (!refreshInProgress) return;
		
		// Wait a bit before restoring the original value
		if (needsRefresh) {
			needsRefresh = false;
			// Schedule the restoration for the next tick
			return;
		}
		
		// Restore original value
		LOGGER.info("Auto-refresh complete: restoring scale to {}", originalScaleFactor);
